...
public class QuestionnaireController {
    ...
    public ResponseEntity<QuestionnaireResponse> archiver(
            @PathVariable Long id) {

        return ResponseEntity.ok(
                questionnaireService.archiver(id));
    }
    @PatchMapping("/{id}/restaurer")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<QuestionnaireResponse> restaurer(
            @PathVariable Long id) {

        return ResponseEntity.ok(
                questionnaireService.restaurer(id));
    }
    @GetMapping("/archives")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<List<QuestionnaireResponse>> archives() {

        return ResponseEntity.ok(
                questionnaireService.listerArchives());
    }
    @GetMapping("/statut/{statut}")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<List<QuestionnaireResponse>> statut(
            @PathVariable String statut) {

        return ResponseEntity.ok(
                questionnaireService.listerParStatut(statut));
    }

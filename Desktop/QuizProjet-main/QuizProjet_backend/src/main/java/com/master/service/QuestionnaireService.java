...
public class QuestionnaireService {
    ...
    // ============ DELETE ============
    // ============ ARCHIVER ============
    public QuestionnaireResponse archiver(Long id) {
    ...
    // ============ RESTAURER ============
    public QuestionnaireResponse restaurer(Long id) {

        Questionnaire questionnaire = questionnaireRepository.findById(id)
                .orElseThrow(() ->
                        new EntityNotFoundException("Questionnaire non trouvé : " + id));

        questionnaire.setStatut(Questionnaire.Statut.BROUILLON);

        return toResponse(questionnaireRepository.save(questionnaire));
    }
    // ============ QUESTIONNAIRES ARCHIVES ============
    @Transactional(readOnly = true)
    public List<QuestionnaireResponse> listerArchives() {

        return questionnaireRepository
                .findByStatut(Questionnaire.Statut.ARCHIVE)
                .stream()
                .map(this::toResponse)
                .toList();
    }
    @Transactional(readOnly = true)
    public List<QuestionnaireResponse> listerParStatut(String statut) {

        Questionnaire.Statut s =
                Questionnaire.Statut.valueOf(statut.toUpperCase());

        return questionnaireRepository.findByStatut(s)
                .stream()
                .map(this::toResponse)
                .toList();
    }
    public void supprimer(Long id) {
        ...
        Questionnaire questionnaire = questionnaireRepository.findById(id)
                .orElseThrow(() ->
                        new RuntimeException("Questionnaire introuvable"));
